...
long? veryLastCaptureIndex, lastCaptureIndex = 0;
veryLastCaptureIndex = lastCapture();
for(;;)
{
	MemoryUpdate();
 
WindowChatChannel    chatLocal = Sanderling.MemoryMeasurementParsed?.Value?.WindowChatChannel?.FirstOrDefault(windowChat => windowChat?.Caption?.RegexMatchSuccessIgnoreCase("local") ?? false);
lastCaptureIndex = lastCapture();
if (veryLastCaptureIndex != lastCaptureIndex)
    {
    Host.Log("Captured");
    veryLastCaptureIndex = lastCaptureIndex;
    }
    if (ListOverviewEntry != null) 
    foreach(var	OverviewEntry in ListOverviewEntry) {
      if (OverviewEntry?.Name != null)
      { 
      //Host.Log(OverviewEntry?.Name.RegexMatchSuccessIgnoreCase("acceleration"));
      if (OverviewEntry?.Name.RegexMatchSuccessIgnoreCase("acceleration") == false)
        {
        Host.Log("Enemy: " + OverviewEntry?.Name +" "+ OverviewEntry?.Type + " , Distance: " + OverviewEntry?.DistanceMax);
...
Parse.IOverviewEntry[] ListOverviewEntry =>
...
var listchatText  =
        ...
        ?.SelectMany(windowChatChannel => windowChatChannel?.LabelText
?.Select(c => new {text = c?.Text, flag = c?.Text?.RegexMatchSuccess(@"has been", RegexOptions.IgnoreCase) ?? false, id = c?.Id, index = c?.InTreeIndex})
            )?.ToArray();
//var	aaa = chatLocal.LabelText?.FirstOrDefault(c => c?.Text?.RegexMatchSuccess(@"captured", RegexOptions.IgnoreCase) ?? false).InTreeIndex;
if (listchatText != null)
foreach(var chatText in listchatText) {
            if (chatText.flag)
            {
                ii = chatText.id;
//                Host.Log(chatText);
